...
#       --data-model-dir connectedhomeip/data_model \
#       --out-proto matter_data_model.proto \
#       --out-bin-dir ./output
#
...
def _discover_versions(data_model_dir: str) -> list[str]:
    """Return sorted list of version sub-directory names (e.g. ['1.0', '1.1'])."""
    versions = []
    for entry in os.scandir(data_model_dir):
...
def _xml_files_for_version(data_model_dir: str, version: str) -> list[str]:
    return glob.glob(
        os.path.join(data_model_dir, version, "**", "*.xml"), recursive=True
    )
...
def generate_proto_file(
    ...
    with open(output_proto, "w") as f:
        ...
        for i, t in enumerate(all_types, 1):
            f.write(f"  TYPE_{_clean_type_name(t)} = {i};\n")
        f.write("}\n\n")
